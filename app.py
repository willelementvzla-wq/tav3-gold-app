"""
TAV3-ST-QQE PRO v4 — Multi-instrument trading dashboard
- Menu principal ARRIBA (instrumento + tipo de grafico + temporalidades)
- Sidebar solo para parametros tecnicos avanzados
- Pisos y techos automaticos del dia (rechazos fuertes)
- Forex completo + Metales + Crypto + Indices + Commodities
- Senales calculadas en TF maestro (3M default), proyectadas al TF visible (1M)
"""
...
<style>
    ...
    /* Selectboxes del menu principal mas compactos */
    div[data-testid="stSelectbox"] label { font-size: 0.7rem !important; color: #787b86 !important; margin-bottom: 0px !important; }
    div[data-testid="stSelectbox"] > div > div { font-size: 0.8rem !important; min-height: 32px !important; }
...
# ============================================================
# CATALOGO COMPLETO DE INSTRUMENTOS
# ============================================================
INSTRUMENTS = {
    "🥇 Metales": {
    ...
    "📊 Indices": {
        ...
        "Dow Jones US30 (^DJI)": "^DJI",
        "Dow Futures (YM=F)": "YM=F",
        "Nasdaq (^IXIC)": "^IXIC",
    ...
    "🛢️ Commodities": {
        "Petroleo WTI (CL=F)": "CL=F",
        "Petroleo Brent (BZ=F)": "BZ=F",
...
SIGNAL_TF_OPTIONS = ["1m", "3m", "5m", "15m"]

# ============================================================
# MENU PRINCIPAL ARRIBA (5 columnas)
# ============================================================
st.markdown('<div class="menu-bar">', unsafe_allow_html=True)

m1, m2, m3, m4, m5 = st.columns([1.2, 2, 1.5, 1, 1])
...
with m2:
    sym_label = st.selectbox(
        f"📈 Instrumento ({len(INSTRUMENTS[category])})",
        list(INSTRUMENTS[category].keys()),
        index=0,
        key="sym",
    )
...
with m5:
    signal_tf = st.selectbox(
        ...
        index=1,
        key="sigtf",
        help="TF maestro donde se calculan las senales BUY/SELL",
    )
...
# ============================================================
# SIDEBAR (parametros avanzados)
# ============================================================
with st.sidebar:
    st.markdown("### ⚙️ Avanzado")
    ...
    st.markdown("##### 🧱 Pisos / Techos")
    show_sr = st.checkbox("Mostrar S/R del día", value=True)
    sr_window = st.slider("Sensibilidad pivots", 3, 15, 5,
                          help="Velas a cada lado para detectar pivot. Menor = más niveles, mayor = solo los más fuertes")
    sr_max_levels = st.slider("Máx niveles a mostrar", 2, 10, 6)
    sr_min_touches = st.slider("Toques mínimos para 'fuerte'", 1, 4, 2)
...
# ============================================================
# PISOS Y TECHOS DEL DIA (Soportes/Resistencias por pivots)
# ============================================================
def find_pivots(df_today, window=5):
    """Encuentra pivots high (techos) y pivots low (pisos) del dia
       Un pivot high = vela cuyo high es mayor que las N velas a cada lado
       Un pivot low = vela cuyo low es menor que las N velas a cada lado"""
    if len(df_today) < window * 2 + 1:
        return [], []
    highs = df_today['High'].values
    lows = df_today['Low'].values
    pivots_h = []  # (timestamp, price)
    pivots_l = []
    for i in range(window, len(df_today) - window):
        is_pivot_h = all(highs[i] >= highs[i-j] for j in range(1, window+1)) and \
                     all(highs[i] >= highs[i+j] for j in range(1, window+1))
        is_pivot_l = all(lows[i] <= lows[i-j] for j in range(1, window+1)) and \
                     all(lows[i] <= lows[i+j] for j in range(1, window+1))
        if is_pivot_h:
            pivots_h.append((df_today.index[i], float(highs[i])))
        if is_pivot_l:
            pivots_l.append((df_today.index[i], float(lows[i])))
    return pivots_h, pivots_l

def cluster_levels(pivots, df_today, tolerance_pct=0.0015):
    """Agrupa pivots cercanos en niveles fuertes.
       Un nivel = grupo de pivots dentro de tolerance_pct.
       Retorna lista [(precio_promedio, n_toques, primera_vez, ultima_vez)]"""
    if not pivots:
        return []
    avg_price = df_today['Close'].mean()
    tol = avg_price * tolerance_pct  # tolerancia absoluta
    pivots_sorted = sorted(pivots, key=lambda x: x[1])
    clusters = []
    current_cluster = [pivots_sorted[0]]
    for p in pivots_sorted[1:]:
        if abs(p[1] - current_cluster[-1][1]) <= tol:
            current_cluster.append(p)
        else:
            if current_cluster:
                prices = [x[1] for x in current_cluster]
                times = [x[0] for x in current_cluster]
                clusters.append({
    ...
    # Ordenar por fuerza (toques) descendente
    return sorted(clusters, key=lambda x: x['touches'], reverse=True)

def get_today_data(df, tf_min):
    """Filtra datos del dia actual UTC"""
    if df.empty:
        return df
...
if df.empty or len(df) < 50:
    st.error(f"❌ No se cargaron datos para {sym_label}. Yahoo puede no soportar este símbolo en {tf_label}. Prueba con otra temporalidad o instrumento.")
    st.stop()

if df_signal.empty or len(df_signal) < 50:
    st.warning(f"⚠️ No hay datos suficientes en {signal_tf} para calcular señales.")
    df_signal = df.copy()
...
# Pisos y techos del dia
df_today = get_today_data(df, tf_minutes)
sr_resistances = []
sr_supports = []
if show_sr and len(df_today) > sr_window * 2 + 1:
    pivots_h, pivots_l = find_pivots(df_today, sr_window)
    res_clusters = cluster_levels(pivots_h, df_today)
    sup_clusters = cluster_levels(pivots_l, df_today)
    sr_resistances = res_clusters[:sr_max_levels]
    sr_supports = sup_clusters[:sr_max_levels]
...
sessions_html = ""
for sname, scolor in [("Sidney", "#FF6B35"), ("Tokio", "#FFD700"),
...
market_status = '<span style="color:#26a69a;">● ABIERTO</span>' if market_open else '<span style="color:#EF5350;">● CERRADO</span>'
...
if "JPY" in sym_code or "INR" in sym_code or "MXN" in sym_code:
    ...
    fmt = "{:,.2f}"
elif "=X" in sym_code and "USD" not in sym_code[:3]:
    fmt = "{:,.5f}"
elif "=X" in sym_code:
    fmt = "{:,.5f}"
else:
...
fig = make_subplots(rows=n_rows, cols=1, shared_xaxes=True,
                    vertical_spacing=0.015, row_heights=heights)
...
if chart_type == "🕯️ Velas":
    fig.add_trace(go.Candlestick(x=df_show.index,
        open=df_show['Open'], high=df_show['High'],
        low=df_show['Low'], close=df_show['Close'], name='Precio',
    ...
    fig.add_trace(go.Candlestick(x=df_show.index,
        ...
        low=ha_view['low'], close=ha_view['close'], name='Heikin Ashi',
        increasing_line_color=inc_color, decreasing_line_color=dec_color,
        increasing_fillcolor=inc_color, decreasing_fillcolor=dec_color,
        line=dict(width=1), showlegend=False), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(x=df_show.index, y=df_show['Close'],
        ...
        name='Precio', showlegend=False), row=1, col=1)
    ...
    fig.add_trace(go.Ohlc(x=df_show.index,
        open=df_show['Open'], high=df_show['High'],
        low=df_show['Low'], close=df_show['Close'], name='OHLC',
        increasing_line_color=inc_color, decreasing_line_color=dec_color,
        showlegend=False), row=1, col=1)
elif chart_type == "⬜ Velas huecas":
    fig.add_trace(go.Candlestick(x=df_show.index,
        open=df_show['Open'], high=df_show['High'],
        low=df_show['Low'], close=df_show['Close'], name='Hollow',
        increasing_line_color=inc_color, decreasing_line_color=dec_color,
        increasing_fillcolor='rgba(0,0,0,0)', decreasing_fillcolor=dec_color,
        line=dict(width=1.5), showlegend=False), row=1, col=1)
...
# ===== PISOS Y TECHOS DEL DIA =====
if show_sr:
    chart_x_start = df_show.index[0]
    chart_x_end = df_show.index[-1]
    # Resistencias (techos) - rojo
    for level in sr_resistances:
        is_strong = level['touches'] >= sr_min_touches
        line_width = 2.5 if is_strong else 1.5
        line_dash = 'solid' if is_strong else 'dash'
        opacity = 0.9 if is_strong else 0.55
        # Linea horizontal
        fig.add_shape(type="line",
                      x0=max(level['first'], chart_x_start), x1=chart_x_end,
                      y0=level['price'], y1=level['price'],
                      line=dict(color='#FF1744', width=line_width, dash=line_dash),
                      opacity=opacity, row=1, col=1)
        # Etiqueta
        label_text = f"R {fmt.format(level['price'])} · {level['touches']}x"
        fig.add_annotation(
            x=chart_x_end, y=level['price'],
            text=label_text, showarrow=False,
            xanchor='left', yanchor='middle',
            font=dict(color='#FF1744', size=9, family='Arial Black'),
            bgcolor='rgba(19,23,34,0.85)',
            bordercolor='#FF1744', borderwidth=1, borderpad=2,
            row=1, col=1,
        )
    # Soportes (pisos) - verde
    for level in sr_supports:
        is_strong = level['touches'] >= sr_min_touches
        line_width = 2.5 if is_strong else 1.5
        line_dash = 'solid' if is_strong else 'dash'
        opacity = 0.9 if is_strong else 0.55
        fig.add_shape(type="line",
                      x0=max(level['first'], chart_x_start), x1=chart_x_end,
                      y0=level['price'], y1=level['price'],
                      line=dict(color='#00E676', width=line_width, dash=line_dash),
                      opacity=opacity, row=1, col=1)
        label_text = f"S {fmt.format(level['price'])} · {level['touches']}x"
        fig.add_annotation(
            x=chart_x_end, y=level['price'],
            text=label_text, showarrow=False,
            xanchor='left', yanchor='middle',
            font=dict(color='#00E676', size=9, family='Arial Black'),
            bgcolor='rgba(19,23,34,0.85)',
            bordercolor='#00E676', borderwidth=1, borderpad=2,
            row=1, col=1,
        )
...
fig.update_layout(
    ...
    height=chart_height,
    margin=dict(l=8, r=80, t=8, b=8),  # margen derecho mayor para etiquetas S/R
    showlegend=True,
...
for r in range(1, n_rows + 1):
    fig.update_xaxes(gridcolor='#1e222d', gridwidth=1, zeroline=False,
                     showspikes=True, spikecolor='#787b86', spikethickness=1,
                     spikedash='solid', spikemode='across', row=r, col=1)
    fig.update_yaxes(gridcolor='#1e222d', gridwidth=1, zeroline=False,
                     showspikes=True, spikecolor='#787b86', spikethickness=1,
                     spikedash='solid', side='right', row=r, col=1)
...
if play_sound and last_signal_time is not None and last_signal_type is not None:
    ...
    <style>
        .signal-banner {{ background: linear-gradient(90deg, #00E676 0%, #00C853 100%); color: #000; padding: 8px 16px; border-radius: 4px; font-weight: bold; text-align: center; font-size: 0.9rem; margin: 4px 0; }}
        .signal-banner-sell {{ background: linear-gradient(90deg, #FF1744 0%, #D50000 100%); color: white; padding: 8px 16px; border-radius: 4px; font-weight: bold; text-align: center; font-size: 0.9rem; margin: 4px 0; }}
    </style>
    """, height=60)
...
# ============================================================
# SECCION INFERIOR: S/R + Senales (en columnas)
# ============================================================
col_sr, col_sig = st.columns(2)

with col_sr:
    with st.expander(f"🧱 Pisos / Techos del día ({len(sr_resistances) + len(sr_supports)} niveles)", expanded=False):
        if sr_resistances:
            st.markdown("**🔴 Resistencias (Techos)**")
            res_data = [{
                'Precio': fmt.format(r['price']),
                'Toques': r['touches'],
                'Distancia': f"{((r['price']/last_price - 1) * 100):+.2f}%",
                'Primer toque': r['first'].strftime('%H:%M UTC'),
            } for r in sr_resistances]
            st.dataframe(pd.DataFrame(res_data), use_container_width=True, hide_index=True)
        if sr_supports:
            st.markdown("**🟢 Soportes (Pisos)**")
            sup_data = [{
                'Precio': fmt.format(s['price']),
                'Toques': s['touches'],
                'Distancia': f"{((s['price']/last_price - 1) * 100):+.2f}%",
                'Primer toque': s['first'].strftime('%H:%M UTC'),
            } for s in sr_supports]
            st.dataframe(pd.DataFrame(sup_data), use_container_width=True, hide_index=True)
        if not sr_resistances and not sr_supports:
            st.info("No se detectaron pivots aún. Necesita más velas del día.")
    ...
    with st.expander(f"📋 Últimas señales ({signal_tf})", expanded=False):
        ...
        for idx in df_signal.index[short_cond_sig]:
            signals.append({'Hora UTC': idx, 'Tipo': '🔴 SELL',
                            'Precio': df_signal.loc[idx, 'Close']})
        if signals:
            df_sig_tbl = pd.DataFrame(signals).sort_values('Hora UTC', ascending=False).head(15)
...
st.caption(f"📡 Yahoo Finance · Auto-refresh 30s · Delay ~15min · Pisos/Techos del día detectados con pivots de {sr_window} velas a cada lado")
